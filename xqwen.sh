...
run_command() {
    ...
    # Tangkap stdout dan stderr secara bersamaan dengan unbuffered output
    output=$(mktemp)
    eval "$cmd" > >(tee -a "$output") 2> >(tee -a "$output" >&2)
    status=$?
    timestamp=$(date '+%Y-%m-%d %H:%M:%S')

    # Baca hasil output dari file sementara
    output_content=$(cat "$output")
    rm -f "$output"

    if [ $status -ne 0 ]; then
        echo -e "[Step $step] [ERROR] ($timestamp) : $output_content" | tee -a "$LOG_FILE"
        ERROR_OCCURRED=1
        ...
        echo -e "[Step $step] [SUCCESS] ($timestamp) : $output_content" | tee -a "$LOG_FILE"
    fi
}
